          ...
          if (!this.isProcessingInternal) {
            ...
            this.globalContainer.getCleanerContainer().getDuplicateCleaner().cleanDuplicates()
              .then(() => {
                ...
                logging.info(NAMESPACE, 'Finished processing job store/details duplicates')
              }).catch((error) => {
                this.isProcessingInternal = false
                logging.error(NAMESPACE, `An error occurred while processing store/details duplicates::${error}`)
              })
          } else {
